...
static void insert_node(list *pp, node *pnode)
{
    if (pnode == NULL) {
...
static void append_node(list *pp, node *pnode)
{
    if (pnode == NULL) {
...
static void remove_node(list *pp, node *pnode)
{
    if (pnode == NULL || isnull(*pp))
        return;
    if (pnode == *pp) {
        *pp = pnode->next;
        free_node(pnode);
    }
    for (node *prev = *pp; prev->next != NULL; prev = prev->next) {
        if (prev->next == pnode) {
